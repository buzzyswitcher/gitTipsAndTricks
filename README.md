...
- Чтобы удалить ветку из **локального** репозитория нужно выполнить `$ git branch -d <branch_name>`. Если ветка, которую вы хотите удалить еще не была смержена, то команду нужно выполнить с опцией `-D` : `$ git branch -D <branch_name>`
